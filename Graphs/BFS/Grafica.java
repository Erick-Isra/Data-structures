...
/** Programa que crea una grafica con las clases Grafica y implementa BFS
...
public class Grafica<T> {
    ...
    */
    ...
    /**
     * Metodo que nos da el arbol generador
     * @param s el identificador del vertice con el que trabajara     
     */
    public void algoritmoBFS( String s ) throws Exception{
        ...
        Cola <Vertice> ColaVisitados = new Cola<>();
        ColaVisitados.encolar(darVertice(s));

        while (!(ColaVisitados.esVacia())) {
            Vertice u = ColaVisitados.darElementoInicio();
            ColaVisitados.desencolar();

            LinkedList<String> vecindad = this.darVecindad(u.identificador);

            while (vecindad.size() != 0) {
                String v = vecindad.getFirst();
                vecindad.remove();
                Vertice w = darVertice(v);
                if(darVisitadoVertice(v) == false){
                    ColaVisitados.encolar(w);
                }
            }
            modificarVisitadoVertice(u.identificador, true);
        }
    }
}/*Fin clase grafica */